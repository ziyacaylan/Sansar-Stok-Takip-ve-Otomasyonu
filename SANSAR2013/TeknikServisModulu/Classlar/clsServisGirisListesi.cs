...
namespace SANSAR2013.TeknikServisModulu.Classlar
{
    class clsServisGirisListesi
    {
        ...
        {
            string query = "SELECT * FROM tbl_arizalilar ";
            return Dbase.TabloCek(query);
        }
        public string ArizaliMusteriKoduCek(string kayit_No)
        {
            string musteri_kodu = "";
            string query = "SELECT * FROM tbl_arizalilar WHERE SERVIS_NO='" + kayit_No + "'";
        ...
        {
            string query = "SELECT * FROM tbl_arizalilar WHERE FIRMA_ADI LIKE '%" + FirmaAdi + "%'";
            DataTable Tablo = Dbase.TabloCek(query);
            return Tablo;
        }
        public DataTable ServisNoSorgula(string kayit_No)
        {
            string query = "SELECT * FROM tbl_arizalilar WHERE SERVIS_NO LIKE '%" + kayit_No + "%'";
            DataTable Tablo = Dbase.TabloCek(query);
            return Tablo;
        }
        public DataTable IrsaliyeNOSorgula(string GIRIS_IRSALIYE_NO)
        {
            string query = "SELECT * FROM tbl_arizalilar WHERE GIRIS_IRSALIYE_NO LIKE '%" + GIRIS_IRSALIYE_NO + "%'";
            DataTable Tablo = Dbase.TabloCek(query);
            return Tablo;
        }
        public DataTable IlkTarihSorgula(string Tarih)
        {
            string query = "SELECT * FROM tbl_arizalilar WHERE SERVIS_GIR_TARIH='" + Tarih + "'";
            DataTable dt = Dbase.TabloCek(query);
            return dt;
        }
        public DataTable IkiTarihArasiSorgula(string Tarih1, string Tarih2)
        {
            string query = "SELECT * FROM tbl_arizalilar WHERE SERVIS_GIR_TARIH BETWEEN '" + Tarih1 + "' AND '" + Tarih2 + "'";
            DataTable dt = Dbase.TabloCek(query);
            return dt;
        }
    }
